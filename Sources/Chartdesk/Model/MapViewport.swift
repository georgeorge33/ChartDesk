...
extension MapSheet {
    ...
    /// The one-degree cells the view covers, for the full coastline.
    ///
    /// Only ever asked once zoomed in far enough that the view is a small patch of the globe,
    /// so the corners of the panel bound it. A patch straddling ±180° is counted by letting
    /// the longitudes run past it and wrapping the cell numbers at the end — the same trick
    /// the tables themselves use, and the only place a sphere still has to think about it.
    func coastlineCells() -> [CoastlineCell] {
        ...
        for corner in corners {
            guard let direction = projection.direction(at: corner) else { continue }
            let coordinate = Coordinate(direction)
            longitudes.append(coordinate.longitude)
            latitudes.append(coordinate.latitude)
        }
        // A corner off the globe means the edge is in view, which is far wider than this is
        // meant for.
        guard longitudes.count == 4 else { return [] }

        if let west = longitudes.min(), let east = longitudes.max(), east - west > 180 {
            longitudes = longitudes.map { $0 < 0 ? $0 + 360 : $0 }
        }

        let west = Int(floor(longitudes.min() ?? 0))
        let east = Int(floor(longitudes.max() ?? 0))
        let south = Int(floor(latitudes.min() ?? 0))
        let north = Int(floor(latitudes.max() ?? 0))

        // A guard against asking for the world a cell at a time, whatever the zoom says.
        guard (east - west + 1) * (north - south + 1) <= 400 else { return [] }

        var cells: [CoastlineCell] = []
        for longitude in west...east {
            var wrapped = longitude
            while wrapped >= 180 { wrapped -= 360 }
            while wrapped < -180 { wrapped += 360 }
            for latitude in south...north {
                cells.append(CoastlineCell(longitude: wrapped, latitude: latitude))
            }
        }
        return cells
    }
