...
//
// What it answered, the first time it was run, so the next reader starts where this left off:
//
//   A row is a [data-index] element, and the list is virtualised — the rows sit absolutely
//   positioned inside a spacer of the full height, and only those near the viewport exist.
//   The button that opens a chart is the one behind img[alt="Preview chart"]; the planner's
//   own save icon is img[alt="Manually saved"] and must be left alone.
//
//   Each row holds its type in a rotated .vertical-writing-lr span — IAC, VAC, SID, SIDPT,
//   STAR, or an airport chart's own code — its title in div.grow.cursor-default, its runways
//   in span.text-gray-400, and its procedure chips as the only buttons in the row with text.
//
//   The selected tab carries border-b-msfs where the others carry border-b-transparent. The
//   Chart Provider buttons, LIDO and FAA, are the same shape as a tab and carry aria-pressed.
//
// Two things this probe looks for and does not find, which is worth knowing before trusting
// its output. The group of sibling buttons it reports as the chart list is the category tab
// strip: a chart row is not a button and its name is not a button's text, so the search goes
// to the nearest thing that fits. And the rows it does report under [role=button] are the
// controls inside a row rather than the row itself.
...
(() => {
    ...
    function scrolling(element) {
        for (let node = element; node && node !== document.body; node = node.parentElement) {
            // Height of its own, as well as more inside than fits. The wrapper holding the
            // rows has none: its children are absolutely positioned, so it measures zero tall
            // with a scrollHeight of the whole list, which reads as a scrolling panel and is
            // not one — setting scrollTop on it moves nothing at all.
            if (node.clientHeight > 40 && node.scrollHeight > node.clientHeight + 8) {
